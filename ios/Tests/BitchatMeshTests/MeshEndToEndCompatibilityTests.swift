...
final class MeshEndToEndCompatibilityTests: XCTestCase {
    private final class FakeTransport: Transport {
        ...
        func currentPeerSnapshots() -> [TransportPeerSnapshot] { snapshotSubject.value }
    ...
    private final class InMemoryKeychain: KeychainManagerProtocol {
        ...
        func secureClear(_ data: inout Data) { data.removeAll(keepingCapacity: false) }
        func secureClear(_ string: inout String) { string.removeAll(keepingCapacity: false) }
        ...
        func getIdentityKeyWithResult(forKey key: String) -> KeychainReadResult {
            if let value = storage[identityKey(key)] { return .success(value) }
            return .itemNotFound
        }
    ...
    func testDirectMessageUsesMockTransport() {
    ...
    func testRoutedPacketBinaryRoundTrip() {
        let sender = Data(hexString: "0102030405060708") ?? Data()
        let nextHop = Data(hexString: "1112131415161718") ?? Data()
        let recipient = Data(hexString: "2122232425262728") ?? Data()
        let route = [sender, nextHop]
        ...
        let encoded = BinaryProtocol.encode(packet, padding: false)
        XCTAssertNotNil(encoded)

        let decoded = BinaryProtocol.decode(encoded!)
        XCTAssertNotNil(decoded)
        XCTAssertEqual(decoded?.route?.count, 2)
        XCTAssertEqual(decoded?.route?.first, sender)
        XCTAssertEqual(decoded?.route?.last, nextHop)

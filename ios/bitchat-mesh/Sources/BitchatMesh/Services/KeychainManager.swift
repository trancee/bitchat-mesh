...
final class KeychainManager: KeychainManagerProtocol {
    ...
    init(configuration: MeshConfiguration = .default) {
        self.service = configuration.keychainService
        self.appGroup = configuration.keychainAccessGroup
    }
    ...
    private func saveData(_ data: Data, forKey key: String) -> Bool {
        ...
        func attempt(addAccessGroup: Bool) -> OSStatus {
            var query = base
            if addAccessGroup, let appGroup { query[kSecAttrAccessGroup as String] = appGroup }
    ...
    private func retrieveData(forKey key: String) -> Data? {
        ...
        func attempt(withAccessGroup: Bool) -> OSStatus {
            var q = base
            if withAccessGroup, let appGroup { q[kSecAttrAccessGroup as String] = appGroup }
    ...
    func deleteAllKeychainData() -> Bool {
        ...
        if searchStatus == errSecSuccess, let items = result as? [[String: Any]] {
            for item in items {
                ...
                if let appGroup, accessGroup == appGroup {
                    shouldDelete = true

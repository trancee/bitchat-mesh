...
class PacketRelayManager(private val myPeerID: String) {
    ...
    suspend fun handlePacketRelay(routed: RoutedPacket) {
        ...
        if (BuildConfig.DEBUG) Log.d(TAG, "Evaluating relay for packet type ${packet.type} from ${peerID} (TTL: ${packet.ttl})")
        
        // Double-check this packet isn't addressed to us
        if (isPacketAddressedToMe(packet)) {
            if (BuildConfig.DEBUG) Log.d(TAG, "Packet addressed to us, skipping relay")
            return
        }
        
        // Skip our own packets
        if (peerID == myPeerID) {
            if (BuildConfig.DEBUG) Log.d(TAG, "Packet from ourselves, skipping relay")
            return
        }
        
        // Check TTL and decrement
        if (packet.ttl == 0u.toUByte()) {
            if (BuildConfig.DEBUG) Log.d(TAG, "TTL expired, not relaying packet")
            return
        }
        ...
        if (shouldRelay) {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "Relay decision: NOT relaying packet type ${packet.type}")
        }
    }
    ...
    private fun shouldRelayPacket(packet: BitchatPacket, fromPeerID: String): Boolean {
        ...
        if (packet.ttl >= 4u) {
            if (BuildConfig.DEBUG) Log.d(TAG, "High TTL (${packet.ttl}), relaying")
            return true
        }
        ...
        if (networkSize <= 3) {
            if (BuildConfig.DEBUG) Log.d(TAG, "Small network (${networkSize} peers), relaying")
            return true
        }
        ...
        if (BuildConfig.DEBUG) Log.d(TAG, "Network size: ${networkSize}, Relay probability: ${relayProb}, Decision: ${shouldRelay}")
    ...
    private fun relayPacket(routed: RoutedPacket) {
        if (BuildConfig.DEBUG) Log.d(TAG, "🔄 Relaying packet type ${routed.packet.type} with TTL ${routed.packet.ttl}")
     ...
     */
    fun shutdown() {
        if (BuildConfig.DEBUG) Log.d(TAG, "Shutting down PacketRelayManager")
        relayScope.cancel()
    }
}

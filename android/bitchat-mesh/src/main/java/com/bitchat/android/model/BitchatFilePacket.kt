...
data class BitchatFilePacket(
    ...
    fun encode(): ByteArray? {
        try {
            if (BuildConfig.DEBUG) Log.d("BitchatFilePacket", "🔄 Encoding: name=$fileName, size=$fileSize, mime=$mimeType")
        ...
        if (nameBytes.size > 0xFFFF || mimeBytes.size > 0xFFFF) {
            ...
            if (content.size > 0xFFFF) {
                if (BuildConfig.DEBUG) Log.d("BitchatFilePacket", "📦 Content exceeds 65535 bytes (${content.size}); will be split into multiple CONTENT TLVs")
            } else {
                if (BuildConfig.DEBUG) Log.d("BitchatFilePacket", "📏 TLV sizes OK: name=${nameBytes.size}, mime=${mimeBytes.size}, content=${content.size}")
            }
        ...
        val result = buf.array()
            if (BuildConfig.DEBUG) Log.d("BitchatFilePacket", "✅ Encoded successfully: ${result.size} bytes total")
            return result
        } catch (e: Exception) {
    ...
    companion object {
        fun decode(data: ByteArray): BitchatFilePacket? {
            if (BuildConfig.DEBUG) Log.d("BitchatFilePacket", "🔄 Decoding ${data.size} bytes")
            try {
                var off = 0
                ...
                if (BuildConfig.DEBUG) Log.d("BitchatFilePacket", "✅ Decoded: name=$n, size=$s, mime=$m, content=${c.size} bytes")
                return result
            } catch (e: Exception) {

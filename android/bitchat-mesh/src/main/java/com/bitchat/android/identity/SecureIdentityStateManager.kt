...
class SecureIdentityStateManager(private val context: Context) {
        ...
        return try {
            ...
            if (privateKeyString != null && publicKeyString != null) {
                ...
                if (privateKey.size == 32 && publicKey.size == 32) {
                    if (BuildConfig.DEBUG) Log.d(TAG, "Loaded static identity key from secure storage")
                ...
                if (BuildConfig.DEBUG) Log.d(TAG, "No static identity key found in storage")
                null
            }
        } catch (e: Exception) {
    ...
    fun saveStaticKey(privateKey: ByteArray, publicKey: ByteArray) {
        try {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "Saved static identity key to secure storage")
        } catch (e: Exception) {
    ...
    fun loadSigningKey(): Pair<ByteArray, ByteArray>? {
        return try {
            ...
            if (privateKeyString != null && publicKeyString != null) {
                ...
                // Validate key sizes
                if (privateKey.size == 32 && publicKey.size == 32) {
                    if (BuildConfig.DEBUG) Log.d(TAG, "Loaded Ed25519 signing key from secure storage")
                    Pair(privateKey, publicKey)
                } else {
                ...
                if (BuildConfig.DEBUG) Log.d(TAG, "No Ed25519 signing key found in storage")
                null
            }
        } catch (e: Exception) {
    ...
    fun saveSigningKey(privateKey: ByteArray, publicKey: ByteArray) {
        try {
            ...
            if (BuildConfig.DEBUG) Log.d(TAG, "Saved Ed25519 signing key to secure storage")
        } catch (e: Exception) {
    ...
    // MARK: - Public SharedPreferences Access
